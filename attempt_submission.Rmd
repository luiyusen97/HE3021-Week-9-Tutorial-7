...
```
The logit value returned `r predict_test_value_logit` compared to probit's `r predict_test_value_probit`. Logit is lower since the cumulative normal distribution approaches 1 faster than the logistic CDF. 

## c

The logit partial effect of experience is `r partial_effect_exper_value_logit` compared to probit's `r partial_effect_exper_value_probit`. Since probit approaches 1 faster, the slope is higher, thus a higher marginal effect of experience.
...
&=\frac{\sum^n_{i=1}(z_i-\bar{z})\beta_0+ \beta_1\sum^n_{i=1}(z_i-\bar{z})x_i+\sum^n_{i=1}(z_i-\bar{z})(u_i-\bar{u})}{\sum^n_{i=1}(z_i-\bar{z})x_i} \\ 
...
wald.test(Sigma = vcov(reduced_form_educ), b = coef(reduced_form_educ), Terms = 3)
```
The identification assumption in this context means that the coefficient for sibs cannot be 0, so that we do not have a misidentified variable acting as an instrumental variable. To test for it, an LM-test was conducted, where the null hypothesis is that the coefficient is zero under $\alpha=0.05$. The p-value returned was `r wald.test(Sigma = vcov(reduced_form_educ), b = coef(reduced_form_educ), Terms = 3)$result$chi2[3]`<0.05, so we have sufficient evidence to reject the null hypothesis. The identification assumption is reasonable to hold.
...
display
```
The standard error for educ/brthord is `r display$coefficients[2,2]`.  
The standard error for sibs is `r display$coefficients[2,3]`.  
...
\end{align}
where $R^2$ is from the regression of education on sibs. Since sibs is significantly correlated with education, then this is close to 1, thus the variance of $\hat{\beta}_1$ is higher than if we were to just use sibs as an IV for educ. This is the same for $\hat{\beta}_2$ as well, since sibs is correlated with educ.

## e

The correlation between $\hat{educ}$ and sibs is `r cor(x = reduced_form_educ$fitted.values, y = reduced_form_educ$model$sibs)`. Thus, as explained in d, multicollinearity problems are likely.